...
3. **`setup-shipping-labels-bucket.sql`** — Bucket étiquettes (si utilisé).
4. **`setup-storage-policies.sql`** — Politiques Storage pour `device-photos` (adapter si besoin).
5. **`setup-all-tables-rls.sql`** — RLS unifié (catalogue public en lecture, reste via service role).
...
- Voir la doc métier dans `Project.md` si besoin.
